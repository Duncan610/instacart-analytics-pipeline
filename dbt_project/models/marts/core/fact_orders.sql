/*
  ...
  WHY INCREMENTAL?
  On first run: builds entire table (full-refresh).
  On daily runs: only processes NEW orders added since
  last run — much cheaper than rebuilding 3.4M rows daily.

  Star schema:
       dim_dates
           |
  dim_users — fact_orders
           |
       (via fact_order_products → dim_products)
*/
...
SELECT
    ...
    o.order_day_of_week AS date_key,  -- → dim_dates
    ...
    li.unique_items,
    li.reordered_items,
    ROUND(li.order_reorder_rate::NUMERIC,4) AS order_reorder_rate,
